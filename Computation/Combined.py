...
## Chinese Stock Version
START_DATE_Chinese = '2016-05-18'
...
def run_daily_update():
    logging.info("Running daily update US...")
    daily_update = DailyUpdateData(START_DATE, DATA_FILE_RAW, DATA_FILE)
    daily_update.update_and_process_data()
    logging.info("Daily update completed US Stock.")

    logging.info("Running daily update Chinese...")
    daily_update_chinese = DailyUpdateDataChinese(START_DATE_Chinese, DATA_FILE_RAW_Chinese, DATA_FILE_Chinese)
    daily_update_chinese.update_and_process_data()
    logging.info("Daily update completed Chinese Stock.")
...
scheduler.start()
logging.info("Scheduler started, will update after 12 hours...")
...
def run_model_endpoint():
    ...
    if input_data.get("useChinese") is True:
        try:
            model = PortfolioModelChinese(input_data)
    ...
    elif input_data.get("useChinese") is False:
        try:
        ...
        return jsonify({"error": "Invalid 'useChinese' value. Must be True or False."}), 400
